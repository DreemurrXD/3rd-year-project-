clf
m1 = 1.7875;
m2 = 1.0285;
m3 = 0.3575;
L1 = 0.29412;
L2 = 0.366795;
g = 9.81;

m3_new = 2.3575;

Ts = 0.01;
%global data_in;



%serial input

import mlreportgen.utils.*
%new_character = normalizeString(str)
%closing all serial ports so you don't have to reset the connection each
%time
search = serialportfind
delete(search)

%change the com port depending on where you plug in the device
s = serialport("COM4", 115220);
P = [];
P2 = [];
Pyz = [];
array_check = [];
m3_state_input = [];
length_of_recording = 0;
n = 1;
disp("start")
while n==1
    response = readline(s);
    %array_check = [array_check, response]
    if response == ("stop"+ sprintf('\r'))
        n = 2;
    end
    if response == ("sensor1" + sprintf('\r'))
        %disp("P1 detected")
        response = readline(s);
        new_character = normalizeString(response);
        if isnumeric(str2double(new_character)) == 1
            P = [P, str2double(new_character)];
            %disp("it's a number");
        end
    end

    if response == ("sensor2" + sprintf('\r'))
        %disp("P2 detected")
        response = readline(s);
        new_character = normalizeString(response);
        if isnumeric(str2double(new_character)) == 1
            P2 = [P2, str2double(new_character)];
            %disp("it's a number 2");
        end
    end   

    if response == ("sensorYZ" + sprintf('\r'))
        %disp("P2 detected")
        response = readline(s);
        new_character = normalizeString(response);
        if isnumeric(str2double(new_character)) == 1
            Pyz = [Pyz, str2double(new_character)];
            %disp("it's a number yz");
        end
    end   

    if response == ("length_of_recording" + sprintf('\r'))
        %disp("length_of_recording")
        response = readline(s);
        new_character = normalizeString(response);
        if isnumeric(str2double(new_character)) == 1
            length_of_recording = str2double(new_character)
            %disp("it's the length");
        end
    end 

    if response == ("m3_input" + sprintf('\r'))
        %disp("m3_input")
        response = readline(s);
        new_character = normalizeString(response);
        if isnumeric(str2double(new_character)) == 1
            m3_state_input = [m3_state_input, str2double(new_character)];
            %disp("it's a number");
        end
    end

end
length_of_recording =round(length_of_recording, 1);
disp("done")
disp(length_of_recording)
%disp(P)
%disp(P2)
%disp("vector")
%disp(P')
hold on
plot(P','r')
plot(P2','b')
plot(Pyz','g')
title('Plot of raw input,  r = upper, b = lower, g=upper in yz plane')
hold off

data_in = P;
data_in2 = P2;
data_inYZ = Pyz;
%length of recording already assigned










%data_in = 0:0.04:40;
%time_in = 0:0.025:25;

%data_in = timeseries(data_in2,time_in);
simout_model_based = 'Simulation_1_3_dof';
mdlWks = get_param('Simulation_1_3_dof','ModelWorkspace');    
clear(mdlWks)
assignin(mdlWks, 'data_in',data_in)
assignin(mdlWks, 'data_in2', data_in2)
assignin(mdlWks, 'data_inYZ', data_inYZ)
assignin(mdlWks, 'trial_length', length_of_recording)

assignin(mdlWks, 'm1',m1)
assignin(mdlWks, 'm2',m2)
assignin(mdlWks, 'm3',m3)
assignin(mdlWks, 'L1',L1)
assignin(mdlWks, 'L2',L2)
assignin(mdlWks, 'g',g)
assignin(mdlWks, 'm3_new',m3_new)
assignin(mdlWks, 'm3_state_input',m3_state_input)
value = getVariable(mdlWks, 'data_in');
%open_system(simout_model_based)
%load_system(simout_model_based)
%set_param(simout_model_based,SimulationCommand="start")                                                                            
%load_system(simout_model_based)
out = sim(simout_model_based);

disp("done")
%1 is input line and 2 is the output
%disp(out.logsout{1})
%disp(out.logsout{2})
%disp(out.logsout{3})
%disp(out.logsout{4})
%disp(out.logsout{5})
%disp(out.logsout{6})
%disp(out.logsout{7})
%disp(out.logsout{8})
%disp(out.logsout{9})
%disp(out.logsout{10})

%disp("10 is above, 11 is below")

disp(out.logsout{11})
disp(out.logsout{12})
disp(out.logsout{13})
%disp(out.logsout{14})
%disp(out.logsout{15})
%disp(out.logsout{16})
%disp(out.logsout{17})
%disp(out.logsout{18})
%disp(out.logsout{19})
%disp(out.logsout{20})
%disp("20 is above, 21 is below")
%disp(out.logsout{21})

%check = get_param(simout_model_based, 'SimulationStatus')

%excess is the time of an unfinished trial right at the end
excess = mod(2000,(length_of_recording+3));
%total length is the total length of a trial+reset
total_length =length_of_recording+3;

%figure containing input and output of arm 1
figure
hold on
%clock
t = out.logsout{1}.Values.Data;
%signal 1
input_1 = squeeze(out.logsout{8}.Values.Data);
plot(t, input_1,"g");
xlim([1950 2000])
%smoothed_1
ref = squeeze(out.logsout{19}.Values.Data);
plot(t, ref,"b");
xlim([1950 2000])
%output 1
y1 = squeeze(out.logsout{11}.Values.Data);
plot(t, y1, "r");
xlim([1950 2000])
title('Plot of upper arm')
hold off;

%figure containing input and output of arm 2
figure
hold on
%clock 
t = out.logsout{1}.Values.Data;
%signal 1
input_2 = squeeze(out.logsout{9}.Values.Data);
plot(t, input_2,"g");
xlim([1950 2000])
%smoothed 2
ref2 = squeeze(out.logsout{20}.Values.Data);
plot(t, ref2,"b");
xlim([1950 2000])
%output 2
y2 = squeeze(out.logsout{12}.Values.Data);
plot(t, y2, "r");
xlim([1950 2000])
title('Plot of lower arm')
hold off;

%figure containing input and output of arm 1 in the YZ plane
figure
hold on
%clock 
t = out.logsout{1}.Values.Data;
%signal 1
input_yz = squeeze(out.logsout{10}.Values.Data);
plot(t, input_yz,"g");
xlim([1950 2000])
%smoothed 2
refyz = squeeze(out.logsout{21}.Values.Data);
plot(t, refyz,"b");
xlim([1950 2000])
%output 2
yYZ = squeeze(out.logsout{13}.Values.Data);
plot(t, yYZ, "r");
xlim([1950 2000])
title('Plot of upper arm in the yz plane')
hold off;


%Norm error 
figure
hold on;


y1_normed = squeeze(out.logsout{2}.Values.Data);
y2_normed = squeeze(out.logsout{3}.Values.Data);
yYZ_normed = squeeze(out.logsout{4}.Values.Data);
xlim([0 2000])
%disp(length(t))
%disp(length(y1_normed))
%disp(length(y2_normed))
plot(t, y1_normed, "r");
plot(t, y2_normed, "b");
plot(t, yYZ_normed, "g");
title('Sum of the error of each trial')
hold off;




figure
graph1=subplot(1,2,1);
hold on
title('Animation of final controlled movement')
mkr1 = scatter(NaN,NaN,[],"red","filled");
mkr2 = scatter(NaN,NaN,[],"blue","filled");

xlim([-1 1])
ylim([-1 1])
zlim([-1 1])
xlabel('x')
ylabel('y')
zlabel('z')

% Move the marker along the line

line1 = animatedline;
line2 = animatedline;

ax = gca;
%ax.CameraPosition = [1 -1 1];
ax.DataAspectRatio = [1 1 1];

n=1;

graph2=subplot(1,2,2);
hold on
mkr1_g2 = scatter(NaN,NaN,[],"red","filled");
mkr2_g2 = scatter(NaN,NaN,[],"blue","filled");
%hold off
xlim([-1 1])
ylim([-1 1])
zlim([-1 1])
xlabel('x')
ylabel('z')
zlabel('y')

% Move the marker along the line

line1_g2 = animatedline;
line2_g2 = animatedline;

ax2 = gca;
ax2.CameraPosition = [1 -1 1];
ax2.DataAspectRatio = [1 1 1];

%y1 = squeeze(out.logsout{7}.Values.Data);
%disp(length(y1))
%length(y2)
%disp(round((2000-(excess+total_length))*length(y1)))
%round((2000-(excess))*length(y1))
y1 = squeeze(out.logsout{11}.Values.Data);
y2 = squeeze(out.logsout{12}.Values.Data);
yYZ = squeeze(out.logsout{13}.Values.Data);

theta1 = y1(round( ( (2000-(excess+total_length) )/2000)*length(y1) ):round(((2000-(excess+3))/2000)*length(y1))) ;
theta2 = y2(round(((2000-(excess+total_length))/2000)*length(y2) ):round(((2000-(excess+3))/2000)*length(y2))) ;
thetaYZ = yYZ(round(((2000-(excess+total_length))/2000)*length(yYZ) ):round(((2000-(excess+3))/2000)*length(yYZ)));

x_1 =  L1 .* sin((theta1./360)*2*pi);
y_1 =  -L1 .* cos((theta1./360)*2*pi).* cos((thetaYZ./360)*2*pi);
z_1 = L1 .* cos((theta1./360)*2*pi).* sin((thetaYZ./360)*2*pi);

x_2 =  L2 .* sin((theta2./360)*2*pi) + L1 .* sin((theta1./360)*2*pi);
y_2 =  (-L2 .* cos((theta2./360)*2*pi) - L1 .* cos((theta1./360)*2*pi)) .* cos((thetaYZ./360)*2*pi);
z_2 =  (L2 .* cos((theta2./360)*2*pi) + L1 .* cos((theta1./360)*2*pi)) .* sin((thetaYZ./360)*2*pi);


length_of_animation = x_1;
try
    video = VideoWriter("2dof","MPEG-4");   
    video.FrameRate = 30;
    open(video)
catch
    disp("file already exists")
end

interval = floor((length(length_of_animation))/(30*length_of_recording));

record = 1;
catch_check = 0;
while n==1
    for i = 1:length(length_of_animation)

        mkr1.XData = x_1(i);
        mkr1.YData = y_1(i);
        mkr1.ZData = z_1(i);
        line1_x = linspace(0,x_1(i),30);
        line1_y = linspace(0,y_1(i),30);
        line1_z = linspace(0,z_1(i),30);
        addpoints(line1, line1_x, line1_y, line1_z);

        
        mkr1_g2.XData = z_1(i);
        mkr1_g2.YData = x_1(i);
        mkr1_g2.ZData = y_1(i);
        line1_g2_x = linspace(0,z_1(i),30);
        line1_g2_y = linspace(0,x_1(i),30);
        line1_g2_z = linspace(0,y_1(i),30);
        addpoints(line1_g2, line1_g2_x, line1_g2_y, line1_g2_z);
        drawnow limitrate 


        mkr2.XData = x_2(i);
        mkr2.YData = y_2(i);
        mkr2.ZData = z_2(i);
        line2_x = linspace(x_1(i),x_2(i),30);
        line2_y = linspace(y_1(i),y_2(i),30);
        line2_z = linspace(z_1(i),z_2(i),30);
        addpoints(line2, line2_x, line2_y, line2_z);


        mkr2_g2.XData = z_2(i);
        mkr2_g2.YData = x_2(i);
        mkr2_g2.ZData = y_2(i);
        line2_g2_x = linspace(z_1(i),z_2(i),30);
        line2_g2_y = linspace(x_1(i),x_2(i),30);
        line2_g2_z = linspace(y_1(i),y_2(i),30);
        addpoints(line2_g2, line2_g2_x, line2_g2_y, line2_g2_z);
        drawnow limitrate

        if record == 1
            if mod(i,interval)==0
                if i ~= length(length_of_animation)
                    try
                        frame = getframe(gcf);
                        writeVideo(video,frame);
                    catch
                        if catch_check == 0
                            disp("video doesn't exist")
                            catch_check = 1;
                        end
                    end
                
                end
            end
        end

        pause(0.000001)
        clearpoints(line1)
        clearpoints(line2)
        clearpoints(line1_g2)
        clearpoints(line2_g2)
        drawnow limitrate 
    end
    close(video)
    record = 0;
end